...
public class AiChatService {
            ...
            try {
                ...
                for (int round = 0; round < MAX_TOOL_ROUNDS; round++) {
                    ...
                    if (conn.getResponseCode() != 200) {
                        emitter.send(SseEmitter.event().name("error").data("AI 服务返回错误: " + conn.getResponseCode()));
                        emitter.complete();
                        return;
                    }
                        ...
                        while ((line = reader.readLine()) != null) {
                            ...
                            if ("content_block_start".equals(type)) {
                                ...
                                if ("tool_use".equals(block.path("type").asText(""))) {
                                    ...
                                    currentToolUse.put("input_json", new StringBuilder());
                                }
                                ...
                                if ("text_delta".equals(deltaType)) {
                                    ...
                                    if (currentToolUse != null) {
                                        ((StringBuilder) currentToolUse.get("input_json"))
                                ...
                                if (currentToolUse != null) {
                                    ...
                                    toolBlock.put("id", currentToolUse.get("id"));
                                    toolBlock.put("name", currentToolUse.get("name"));
                                    try {
                                        toolBlock.put("input", objectMapper.readTree(
                                                currentToolUse.get("input_json").toString()));
                                    } catch (Exception e) {
                                ...
                                messageStopReason = node.path("delta").path("stop_reason").asText("");
                            }
                        }
                    }

                    if ("end_turn".equals(messageStopReason) || "stop".equals(messageStopReason)) {
                        ...
                        String finalText = extractTextContent(contentBlocks);
                    ...
                    if (!toolUseBlocks.isEmpty()) {
                        ...
                        for (Map<String, Object> tb : toolUseBlocks) {
                            Map<String, Object> block = new HashMap<>();
                            block.put("type", "tool_use");
                            block.put("id", tb.get("id"));
                            block.put("name", tb.get("name"));
                            block.put("input", tb.get("input"));
                        ...
                        for (Map<String, Object> tb : toolUseBlocks) {
                            String toolName = (String) tb.get("name");
                            JsonNode toolInput = (JsonNode) tb.get("input");
                            String toolResult = executeTool(toolName, toolInput);

                            try {
                                emitter.send(SseEmitter.event().name("tool").data(toolName));
                            } catch (Exception e) {
                                return;
                            }
                            ...
                            toolResultMsg.put("role", "user");
                            toolResultMsg.put("content", List.of(
                                    Map.of("type", "tool_result", "tool_use_id", tb.get("id"), "content", toolResult)
                            ));
                            anthropicMessages.add(toolResultMsg);
                        }
                        toolUseBlocks.clear();
                ...
                emitter.send(SseEmitter.event().name("error").data("处理超时"));
                emitter.complete();
            } catch (Exception e) {
    ...
    /**
     * 从 content blocks 中提取纯文本（Map 版本，用于流式）
     */
